...
def register():
    ...
    if form.validate_on_submit():
        ...
        user_find = User.query.filter_by(email=email).first()
        if user_find:
            flash('you already registed')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user_find:
            ...
            return redirect(url_for('login'))
        elif check_password_hash(user_find.password, password):
            login_user(user_find)
            return redirect(url_for('get_all_posts'))
        else:
            flash('password not exist')
            return redirect(url_for('login'))
